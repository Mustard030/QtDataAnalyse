...
df['C1_C4_percentages'] = C1_C4_counts / organic_counts * 100
df['C5_C13_percentages'] = C5_C13_counts / organic_counts * 100
df['C14_C40_percentages'] = C14_C40_counts / organic_counts * 100
df['C40p_percentages'] = C40p_counts / organic_counts * 100

# 确保在C1_C4_Count为0的情况下避免除以零错误
df['C1_C4_percentages'] = df['C1_C4_percentages'].apply(lambda x: 0 if pd.isna(x) else x)
df['C5_C13_percentages'] = df['C5_C13_percentages'].apply(lambda x: 0 if pd.isna(x) else x)
df['C14_C40_percentages'] = df['C14_C40_percentages'].apply(lambda x: 0 if pd.isna(x) else x)
df['C40p_percentages'] = df['C40p_percentages'].apply(lambda x: 0 if pd.isna(x) else x)
...
plt.title('Organic Group Percentage of Organic')
plt.legend()
plt.grid(True)
plt.show()
